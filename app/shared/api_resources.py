...
# Импортируем сервисы
from app.features.categories.services import (create_category, delete_category,
                                              get_all_categories,
                                              get_category_by_id,
                                              update_category)
from app.features.products.services import (create_product, delete_product,
                                            get_all_products,
                                            get_product_by_id, update_product)
from app.features.users.services import (create_user, delete_user,
                                        get_all_users, get_user_by_id,
                                        update_user)

from .swagger import (api, categories_ns, category_create_model,
                      category_model, category_update_model, error_model,
                      pagination_model, product_create_model, product_model,
                      product_update_model, products_ns, success_model,
                      user_model, users_ns)
...
class UserDetail(Resource):
    ...
    def put(self, user_id):
        ...
        try:
            ...
            user = update_user(
                user_id, username, email, first_name, last_name, is_active
            )
            if not user:
                users_ns.abort(404, "Пользователь не найден")
            return {
